...
bp = Blueprint("drainage", __name__, url_prefix="/drainage")
...
def drainage_page():
    ...
    # Fetch only drainage category
    cursor.execute("SELECT * FROM reports WHERE category='Drainage' ORDER BY created_at DESC")
    ...
    return render_template("user/drainage.html", reports=reports, comments=comments_dict)


@bp.route("/interact", methods=["POST"])
def drainage_interact():
    ...
    conn.commit()
    conn.close()
    return redirect(url_for("drainage.drainage_page"))
