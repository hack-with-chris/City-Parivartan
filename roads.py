...
def roads_page():
    ...
    cursor.execute("SELECT * FROM reports WHERE category='Roads' ORDER BY created_at DESC")
    ...
    return render_template("user/roads.html", reports=reports, comments=comments_dict)
...
def roads_interact():
    ...
    conn.commit()
    conn.close()
    return redirect(url_for("roads.roads_page"))
